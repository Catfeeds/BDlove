<?php
class RongCloud_model extends CI_Model {
    ...
    public function __construct() {
        $appKeys = $this->common_function->get_system_value('rongcloud_AppKey');
        $appSecrets = $this->common_function->get_system_value('rongcloud_AppSecret');
        $this->appKey = !empty($appKeys)?$appKeys:"x18ywvqfxjyrc";
        $this->appSecret = !empty($appSecrets)?$appSecrets:"kIg8mMbaSjQ";
        $this->format = 'json';
    }            
    ...
    public function getToken($userId, $name, $portraitUri)
    {
        try {
            ...
            $ret = $this->curl('/user/getToken',$params,'urlencoded','im','POST');
            if (empty($ret))
