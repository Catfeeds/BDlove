...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 rpOptions: [5,15,40,300,500],
			 title: false,
			 reload: true,
			 columnControl: true,
			 pagestat: '共{total}条记录)<span id="statistics">',
		...
		var g = {
				...
				if (p.dataType=='xml')
					{
						...
						p.aa=+$('rows aa',data).text();//自定义aa的值
						p.bb=+$('rows bb',data).text();//自定义aa的值
					}
				else
					p.total = data.total;
			...
			stat=stat.replace(/{aa}/,p.aa);
			stat=stat.replace(/{bb}/,p.bb);
		...
		if (p.columnControl && $('th',g.hDiv).length)
		{
			...
			g.nDiv.innerHTML = "<div id='nDivScroll'><h5>配置表格显示项</h5><ul title='当数据表显示项过多时可滚动鼠标滑轮向下操使'></ul></div>";
