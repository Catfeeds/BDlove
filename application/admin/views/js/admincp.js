...
function openItem(param) {
    ...
    data_str = param.split('|');														//分割传过来的param
    $this = $('div[id^="admincpNavTabs_"]').find('a[data-param="' + param + '"]');		//左侧对应的id=admincpNavTabs_
    if ($('.admincp-container').hasClass('unfold')) {									//左侧菜单是否收起
        $('.sub-menu').hide();					//三级菜单收起
    ...
    $.cookie('workspaceParam', data_str[0] + '|' + data_str[1], { expires: 1 ,path:"/"});	//正在的位置存入cookie
}