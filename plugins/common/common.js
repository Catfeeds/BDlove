...
/*全选*/
function content_checkAll(obj){
	if($(obj).is(':checked')){
		$(obj).parents('table.table-content').find('tbody').find('input[type=checkbox]').attr('checked',true);
	}else{
		$(obj).parents('table.table-content').find('tbody').find('input[type=checkbox]').attr('checked',false);
	}
}
/*列表加载及分页*/
function load_page_to(url){
        $('div.pDiv').find('input.pcur').val(1);
	load_page(url);
}

function load_page(url){
	var curpage = $('div.pDiv').find('input.pcur').val();
	var rp = $('div.pDiv').find('select[name=rp]').val();
	var post_data={curpage:curpage,rp:rp};
	var search_data = $("#formSearch").serialize();
	url_load = url;
	if(url.indexOf('?')==-1){
		url_load = url+'?'+search_data;
	}else{
		url_load = url+'&'+search_data;
	}
	$.ajax({
        ...
        dataType: "html",
        beforeSend:function(){
        	var col_num = $('table.table-content').find('thead').children('tr.text-c').find('th').size();
        	$('table.table-content').find('tbody').html('<tr><td colspan="'+col_num+'"><li class="goods_list mb-20 no-data"><i class="fa fa-exclamation-circle"></i>数据加载中...</li></td></tr>');
        	//$('table.table-content').find('tbody').append('<div class="loading" style="position: fixed;z-index: 3;width: 100%;height:50px;bottom: 40px;left: 0;"></div>');
		},
        success: function(data){
        	if(data.state==403){
        	...
        	//return false;
        	$('table.table-content').find('tbody').html(data);
        	var total =  ($('#total-num').val()==undefined) ?  0 : parseInt($('#total-num').val());
    		var curpage = ($('#per-page-cur').val()==undefined) ?  1 : parseInt($('#per-page-cur').val());
    		var rp = ($('#per-page-rp').val()==undefined) ?  5 : parseInt($('#per-page-rp').val());
    		total_page = Math.ceil(total/rp);
    		if(total_page==0){total_page=1;};
    		curnum_start = (curpage-1)*rp+1;
    		curnum_end = (curpage)*rp;
    		if(curnum_end>total){
    			curnum_end = total;
    		}
    		if(total==0){
    		...
    		$('div.pDiv').find('input.pcur').val(curpage);
    		$('div.pDiv').find('select[name=rp]').val(rp);
    		$('div.pDiv').find('span.pPageStat1').find('span.total').text(total);
    		$('div.pDiv').find('span.ptotal').text(total_page);
    		$('div.pDiv').find('span.pto').text(curnum_end);
    		$('div.pDiv').find('span.pfrom').text(curnum_start);
    		if(curpage>total_page){
            	curpage = curpage-1;
            	$('div.pDiv').find('input.pcur').val(curpage);
            	load_page(url);
    		}
    		if(url=='order_management?op=getList'){
    			load_success();
    			function load_success(){
        	    	$.each($('.table-content').children('tbody').find("tr.text-c"), function(){
        	    		var data = $(this).find('ul').data('info');
    		            //console.log(data);
    		            if (data.shipping_status == 2||data.shipping_status == 7) {
    		                //$(this).find('.shipping_status').addClass("bc_blue");
    		                if (data.total_order_num != data.total_true_num) {
    		                    $(this).find('.order_num').addClass("bc_blue");
    		                }
    		            }
    		            if ((data.order_state != 4 && (data.delivery_state == 3 || data.delivery_state == 2))||data.order_state==2) {
    		                $(this).removeClass("erow");
    		                $(this).addClass("bc_red");
    		                $(this).find('span.stock_supp_price').css('color','#ccc');
    		                $(this).find('a.opa').css('color','#002ACB');
    		                
    		            } else if (data.pay_status == 0 || data.pay_status == 1) {
    		            	if(data.store_order_amount!=''&&data.store_order_amount!=null){
    		            		if(data.store_order_amount<data.goods_amount){
    		            			$(this).removeClass("erow");
    				                $(this).addClass("bc_yellow");
    		            		}
    		            	}
	...
	/*$('table.table-content tbody').load(url_load,post_data,function(response,status,xhr){
		var total =  ($('#total-num').val()==undefined) ?  0 : parseInt($('#total-num').val());
		var curpage = ($('#per-page-cur').val()==undefined) ?  1 : parseInt($('#per-page-cur').val());
		var rp = ($('#per-page-rp').val()==undefined) ?  5 : parseInt($('#per-page-rp').val());
		total_page = Math.ceil(total/rp);
...
function load_page_keydown(e,url){
	   var key = e.which;
	   if(key == 13){
		   var curpage = $('div.pDiv').find('.pcur').val();
		   var curpage_old = parseInt($('#per-page-cur').val());
		   total_page = parseInt($('div.pDiv').find('span.ptotal').text());
		   if (curpage_old != curpage) {
			   if(curpage > total_page){
				   $(this).val(total_page);
			   }
			   load_page(url);
...
function load_page_prev(url){
	var curpage = parseInt($('div.pDiv').find('input.pcur').val());
	if(curpage<=1){
		layer.msg('已经是第一页了！！！',{icon:2});
		return false;
	}
	curpage = curpage-1;
	$('div.pDiv').find('.pcur').val(curpage);
	load_page(url);
}
function load_page_next(url){
	var curpage = parseInt($('div.pDiv').find('input.pcur').val());
	var total_page = parseInt($('div.pDiv').find('span.ptotal').text());
	if(curpage>=total_page){
...
function load_page_first(url){
	var curpage = parseInt($('div.pDiv').find('input.pcur').val());
	if(curpage<=1){
		layer.msg('已经是第一页了！！！',{icon:2});
		return false;
	}
	$('div.pDiv').find('.pcur').val(1);
	load_page(url);
}
function load_page_last(url){
	var curpage = parseInt($('div.pDiv').find('input.pcur').val());
	var total_page = parseInt($('div.pDiv').find('span.ptotal').text());
	if(curpage>=total_page){
		layer.msg('已经是最后一页了！！！',{icon:2});
	...
	$('div.pDiv').find('.pcur').val(total_page);
	load_page(url);
}
