...
class AlipayTradeService {
	...
	function writeLog($text) {
		...
		//$text = characet ( $text );
		file_put_contents ( FCPATH."libraries/alipay/f2fpay/log/log.txt", date ( "Y-m-d H:i:s" ) . "  " . $text . "\r\n", FILE_APPEND );
	}
    ...
    function create_erweimaurl($content,$orderid) {
        require_once ROOTPATH.'libraries/phpqrcode.php';
        $errorCorrectionLevel = 'L';//容错级别
        $matrixPointSize = 8;//生成图片大小
        $outfile=FCPATH."libraries/alipay/img/".$orderid.'.png';
        //生成二维码图片
        QRcode::png($content, $outfile, $errorCorrectionLevel, $matrixPointSize, 2,$saveandprint=false);
        $enc = QRencode::factory($errorCorrectionLevel, $matrixPointSize, 2);
        $enc->encodePNG($content, $outfile, $saveandprint=false);
        $fileurl=base_url("libraries/alipay/img/".$orderid.'.png');
        return $fileurl;
    }
